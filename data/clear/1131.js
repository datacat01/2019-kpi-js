...
(function($) {
    ...
    Drupal.behaviors.searchSubmit = {
        attach: function(context, settings) {
            $('#psu-search-header-block-form:not(.processed)', context).addClass('processed').submit(function(e) {
...
// Fix a strange "race condition" where you load with one screen size and resize
// during load. Somehow, the mobile AND normal classes can be applied at the same
// time. This triggers a 1 second delay where it re-applies the classes and removes
// the other class, fixing this rare edge-case.
jQuery(document).ready(function() {
    ...
    function refreshBodyCheck() {
        if (document.body !== null) {
            width = window.innerWidth;
            if (width < 740) {
                document.body.className += " responsive-layout-mobile";
                document.querySelector('body').classList.remove("responsive-layout-narrow");
                document.querySelector('body').classList.remove("responsive-layout-normal");
            } else if (width < 980) {
                document.body.className += " responsive-layout-narrow";
                document.querySelector('body').classList.remove("responsive-layout-mobile");
                document.querySelector('body').classList.remove("responsive-layout-normal");
            } else {
                document.body.className += " responsive-layout-normal";
                document.querySelector('body').classList.remove("responsive-layout-mobile");
        ...
        attach: function(context, settings) {
            $('#block-psu-feature-article-feature-article-header .node-video, .drupal-embed .node-video, .node-video.node-video_teaser.video-player').each(function() {
                ...
                if (thumbnail.length !== 0) {
                    ...
                    var links = $(this).find('a').not('.read-more');
                    video.hide();
