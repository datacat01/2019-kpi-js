function insertParam(a, b, c, d) {
    ...
    return "" === i && (i = "?"), d ? i = "?" + b + "=" + c + (1 < i.length ? "&" + i.substring(1) : "") : ("" !== i && "?" !== i && (i += "&"), i += b + "=" + (c || "")), h[0] + i + f
}

function registerYoutubePlayers(a) {
    Number(fusionVideoGeneralVars.status_yt) && !0 === window.yt_vid_exists && (window.$youtube_players = [], jQuery(".tfs-slider").each(function() {
        var b = jQuery(this),
            c = b.find("[data-youtube-video-id]").find("iframe").length,
            d = !1;
        b.find("[data-youtube-video-id]").find("iframe").each(function(e) {
            var f = jQuery(this);
            c === e + 1 && void 0 !== a && (d = b.data("flexslider")), window.YTReady(function() {
                window.$youtube_players[f.attr("id")] = new YT.Player(f.attr("id"), {
                    events: {
                        onReady: onPlayerReady(f.parents("li"), d),
                        onStateChange: onPlayerStateChange(f.attr("id"), b)
                    }
                })
            })
        })
...
function onPlayerReady(a, b) {
    return function(c) {
        "yes" === jQuery(a).data("mute") && c.target.mute(), b && setTimeout(function() {
...
function playVideoAndPauseOthers(a) {
    ...
    b.each(function() {
        jQuery(this).attr("id") !== d.attr("id") && void 0 !== window.$youtube_players && window.$youtube_players[jQuery(this).attr("id")].stopVideo()
    }), d.length && ("function" != typeof fusionGetConsent || fusionGetConsent("youtube")) && void 0 !== window.$youtube_players && (!d.parents("li").hasClass("clone") && d.parents("li").hasClass("flex-active-slide") && "yes" === d.parents("li").attr("data-autoplay") && (void 0 === window.$youtube_players || void 0 === window.$youtube_players[d.attr("id")] || "function" != typeof window.$youtube_players[d.attr("id")].playVideo ? setTimeout(function() {
    ...
    }, 2e3) : window.$youtube_players[d.attr("id")].playVideo()), "yes" === c.attr("data-mute") && window.$youtube_players[d.attr("id")].mute()), Number(fusionVideoGeneralVars.status_vimeo) && ("function" != typeof fusionGetConsent || fusionGetConsent("vimeo")) && setTimeout(function() {
        jQuery(a).find("[data-vimeo-video-id] > iframe").each(function() {
            new Vimeo.Player(jQuery(this)[0]).pause()
        }), 0 !== a.slides.eq(a.currentSlide).find("[data-vimeo-video-id] > iframe").length && ("yes" === jQuery(a.slides.eq(a.currentSlide)).data("autoplay") && new Vimeo.Player(a.slides.eq(a.currentSlide).find("iframe")[0]).play(), "yes" === jQuery(a.slides.eq(a.currentSlide)).data("mute") && new Vimeo.Player(a.slides.eq(a.currentSlide).find("iframe")[0]).setVolume(0))
    }, 300), jQuery(a).find("video").each(function() {
...
window.YTReady = function() {
    ...
    jQuery(".fusion-fullwidth.video-background").each(function() {
        ...
        var c, d, e = jQuery(this).attr("src"),
            f = jQuery(this).data("privacy-src"),
            g = !e && f ? f : e;
        g && (Number(fusionVideoGeneralVars.status_vimeo) && 1 <= g.indexOf("vimeo") && jQuery(this).attr("id", "player_" + (a + 1)), Number(fusionVideoGeneralVars.status_yt) && ytVidId(g) && (jQuery(this).attr("id", "player_" + (a + 1)), c = insertParam(g, "enablejsapi", "1", !1), d = insertParam(c, "wmode", "opaque", !1), e ? jQuery(this).attr("src", d) : f && jQuery(this).attr("data-privacy-src", d), window.yt_vid_exists = !0))
