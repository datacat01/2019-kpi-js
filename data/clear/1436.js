            ...
            if (data.success == 1 && data.filtered.length > 0) {
                ...
                if (GSteamBroadcasts.m_fnFilterItemToDisplayFunction) {
                    rgFiltered = GSteamBroadcasts.m_fnFilterItemToDisplayFunction(
                        data.filtered, 'home', 1, GSteamBroadcasts.m_nMaxBroadcasts, {
                            displayed_elsewhere: true
                        }
                    );
                }
                ...
                if (rgFiltered2.length > 0) {
                    ...
                    for (var i = 0; i < rgFiltered2.length && i < GSteamBroadcasts.m_nMaxBroadcasts; i++) {
                        ...
                        var params = {
                            'class': 'store_capsule broadcast_capsule',
                            'href': oItem.app_link
                        };
                        ...
                        var rgImageProperties = {
                            src: oItem.thumbnail_http_address
                        };
                        var rgDefaultImageProperties = {
                            src: oItem.app_capsule_image
                        };
                        ...
                        var rgPlayIconProperties = {
                            src: 'https://steamstore-a.akamaihd.net/public/shared/images/apphubs/play_icon80.png',
                            class: "live_stream_play_icon"
                        };
                        ...
                        $Contents.append($J('<span/>', {
                            'class': 'live_stream_app'
                        }).html(oItem.app_name));
                        $Contents.append($J('<span/>', {
                            'class': 'live_steam_viewers'
                        }).html(Number(oItem.viewer_count).toLocaleString()));
