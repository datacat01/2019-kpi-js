...
GHomepage = {
    ...
    oApplicableSettings: {
        "main_cluster": {
            ...
            "new_on_steam": null,
            "popular_new_releases": "always",
            "games_not_in_library": null,
            "only_current_platform": true,
            "video": true,
            "localized": true,
            "virtual_reality": true,
            "recommended_by_curators": true,
            "hidden": null
        },
        "new_on_steam": {
            "top_sellers": null,
            ...
            "recently_viewed": null,
            "new_on_steam": null,
            "popular_new_releases": null,
            "games_not_in_library": null,
            "only_current_platform": true,
            "video": true,
            "localized": true,
            "virtual_reality": true,
            "recommended_by_curators": null,
            "hidden": null
        },
        "recently_updated": {
            "top_sellers": null,
            "early_access": true,
            "games_already_in_library": null,
            "recommended_for_you": null,
            "prepurchase": null,
            "games": "always",
            "software": true,
        ...
        try {
            ...
            if (rgParams.rgRecommendedGames && rgParams.rgRecommendedGames.length) {
                ...
                for (var i = 0; i < rgRecommendedAppIDs.length; i++) {
                    GHomepage.rgRecommendedGames.push({
                        appid: rgRecommendedAppIDs[i],
                        recommended: true
                    });
                }
            }
            ...
            if (GHomepage.rgCreatorFollowedAppData && rgParams.rgCreatorFollowedAppData['apps'] && rgParams.rgCreatorFollowedAppData['apps'].length) {
                ...
                for (var i = 0; i < rgRecentAppIDs.length; i++) {
                    ...
                    GHomepage.rgRecentAppsByCreator.push({
                        appid: rgRecentAppIDs[i].appid,
    ...
    OnDynamicStoreReady: function() {
        ...
        // More Content
        if (bHaveUser) {
            $J('#content_more').autoloader({
                template_url: 'https://store.steampowered.com/explore/render/',
                recommendations_url: 'https://store.steampowered.com/explore/recommended/',
                additional_url: 'https://store.steampowered.com/explore/more'
            });
        } else {
            $J('#content_more').hide();
    ...
    RenderMainClusterV2: function() {
        ...
        rgDisplayListCombined = GHomepage.FilterItemsForDisplay(
            rgDisplayListCombined, 'home', 6, 15, {
        ...
        for (var i = 0; i < nMainCaps && i < 12; i++) {
            ...
            if (GHomepage.bAutumnSaleMainCap) {
                ...
                var $SmallCap = GHomepage.BuildHomePageGenericCap(GHomepage.bNewRecommendations ? 'summer2018_standardview_recommend_neural' : 'main_cluster_recommended', oItem.appid, oItem.packageid, oItem.bundleid, {
                    capsule_size: 'header',
                    no_hover: true
                });
                $SmallCap.on('mouseenter', (function(index) {
                    return function() {
    ...
    BuildMainCapsuleItem: function(rgItem, strFeatureContext) {
        ...
        var params = {
            'class': rgOptions.class + ' broadcast_capsule',
            'data-manual-tracking': 1
        };
        ...
        if (rgItemData.main_capsule) {
            ...
            if (strImageURL) {
                $ImgCtn.append($J('<div/>', {
                        'class': 'cluster_maincap_fill ' + (rgItemData.package_header ? 'package' : '')
                    })
                    .append(
                        $J('<img/>', {
                            'class': 'cluster_maincap_fill_placeholder',
                            src: 'https://steamstore-a.akamaihd.net/public/images/v6/home/maincap_placeholder_616x353.gif'
                        }),
                        $J('<img/>', {
                            'class': 'cluster_capsule_image cluster_maincap_fill_bg',
                            src: 'https://steamstore-a.akamaihd.net/public/images/blank.gif',
                            'data-image-url': strImageURL
                        }),
                        $J('<img/>', {
                            'class': 'cluster_maincap_fill_header',
                            src: 'https://steamstore-a.akamaihd.net/public/images/blank.gif',
                            'data-image-url': strImageURL
                        })
                    )
        ...
        if (rgAppInfo && rgAppInfo.live_broadcast) {
            $ImgCtn.append($J('<div/>', {
                'class': 'broadcast_live_stream_icon'
            }).append('Live'));
        }
        ...
        if (rgScreenshots && rgScreenshots.length) {
            for (var i = 0; i < 4 && i < rgScreenshots.length; i++) {
                ...
                var $ImgScreenshot = $J('<div/>', {
                    class: 'screenshot'
                }).attr('data-background-image-url', GetScreenshotURL(unScreenshotAppId, screenshot.filename, '.600x338'));

                $ImgCtn.append($ImgScreenshot);

                $elScreenshotThumbWrapper.on('mouseover', function($elTarget) {
                    return function() {
                        $elTarget.addClass('focus')
                    }
                }($ImgScreenshot));
                $elScreenshotThumbWrapper.on('mouseout', function($elTarget) {
                    return function() {
                        $elTarget.removeClass('focus')
                    }
                }($ImgScreenshot));
                ...
                $el.on('hover', function() {
                    ...
                    if ($elInfoDiv) {
                        ...
                        if (rgData['review_summary']) {
                            ...
                            var $elReviewData = $J('<div>', {
                                'class': 'tab_review_summary',
                                "data-tooltip-html": reviewSummary['sReviewScoreTooltip']
                            });
                            $elReviewData.append($J('<div>', {
                                'class': 'title'
                            }).text(rgData.appids ? 'Overall user reviews for items in this bundle:' : 'Overall user reviews:'));
                            $elReviewData.append($J('<span>', {
                                'class': 'game_review_summary ' + reviewSummary['sReviewSummaryClass']
                            }).text(reviewSummary['reviewSummaryDesc']));
                        ...
                        for (var i = 0; i < 4; i++) {
                            if (i < rgScreenshots.length) {
                                ...
                                var $elScreenshot = $J('<div>', {
                                    'class': 'screenshot'
                                }).attr('data-background-image-url', GetScreenshotURL(unAppId, screenshot.filename, '.600x338'));
                                $elInfoDiv.append($elScreenshot);
                            } else if (bMissingScreenshotsMayBeMature) {
                                var $elScreenshot = $J('<div>', {
                                    'class': 'screenshot empty'
                                }).append("<div>").text("This screenshot may contain mature content.");
                                $elInfoDiv.append($elScreenshot);
                            }
                        }
    ...
    RenderFriendsRecentlyPurchased: function() {
        ...
        var rgCapsules = GHomepage.FilterItemsForDisplay(
            GHomepage.rgUserNewsFriendsPurchased, 'home', 4, 8, {
                games_already_in_library: false,
                dlc: false,
                localized: true,
                displayed_elsewhere: false,
    ...
    RenderRecommendedCreatorApps: function() {
        ...
        var rgCapsules = GHomepage.FilterItemsForDisplay(
            GHomepage.rgRecommendedAppsByCreators, 'home', 4, 100, {
                games_already_in_library: false,
                dlc: false,
                localized: true,
                displayed_elsewhere: false
    ...
    RenderTopVRApps: function() {
        ...
        var rgCapsules = GHomepage.FilterItemsForDisplay(
            GHomepage.oDisplayLists.top_vr, 'home', 4, 100, {
                games_already_in_library: false,
                dlc: false,
                localized: true,
                displayed_elsewhere: false
    ...
    RenderSpotlightSection: function() {
        ...
        var rgCapsules = GHomepage.FilterItemsForDisplay(
            specials, 'home', nSpecials, nSpecials, {
                games_already_in_library: false,
                dlc: false,
                localized: true,
                displayed_elsewhere: false,
    ...
    FilterTabs: function() {
        ...
        for (var i = 0; i < rgTabSections.length; i++) {
            ...
            this.FilterCapsules(10, 10, $elTabSection.children(), $elTabSection, {
                games_already_in_library: false,
                only_current_platform: true
            })
        }
    ...
    RenderUnder10: function() {
        ...
        var rgCapsules = GHomepage.FilterItemsForDisplay(
            GHomepage.oDisplayLists.under10, 'home', 4, 8, {
                games_already_in_library: false,
                dlc: false,
                localized: true,
                displayed_elsewhere: false,
    ...
    BuildHomePageHeaderCap: function(strFeatureContext, unAppID, unPackageID) {
        ...
        $CapCtn.append($J('<img/>', {
            src: rgItemData.header
        }));
        $CapCtn.append($J('<div/>', {
            'class': 'home_smallcap_title ellipsis'
        }).html(rgItemData.name));
    ...
    BuildHomePageGenericCap: function(strFeatureContext, unAppID, unPackageID, unBundleID, rgOptions) {
        ...
        var rgImageProperties = {
            src: rgItemData[rgOptions.capsule_size]
        };
        if (rgOptions.lazy)
            rgImageProperties = {
                'data-image-url': rgItemData[rgOptions.capsule_size]
            }
    ...
    RenderRecommendedSpotlight: function(unAppID, strDescription, bNoDSFlag) {
        var $SpotlightCtn = $J('<div/>', {
            'class': 'recommended_spotlight_ctn'
        });

        var params = {
            'class': 'recommended_spotlight broadcast_capsule'
        };
        ...
        $Spotlight.append($J('<div/>', {
            'class': 'recommended_spotlight_cap'
        }).append($J('<img/>', {
            src: strHeaderURL
        })));
        $Spotlight.append($J('<div/>', {
            'class': 'recommended_spotlight_desc'
        }).text(strDescription));
        $Spotlight.append($J('<div/>', {
            'class': 'recommended_spotlight_price'
        }).html(rgItemData.discount_block ? $J(rgItemData.discount_block).addClass('discount_block_spotlight discount_block_large') : '&nbsp;'));
...
CHomeSettings.prototype.RenderCustomizeButton = function() {
    var $Btn = $J('<div/>', {
        'class': 'home_btn home_customize_btn'
    }).text('Customize');
...
CHomeSettings.prototype.DisplayPopup = function($Btn) {
    ...
    this.m_$Popup = $J('<div/>', {
        'class': 'home_viewsettings_popup'
    });
    this.m_$Popup.append($J('<div/>', {
        'class': 'home_viewsettings_instructions'
    }).text('Select the types of products that you wish to see in this section'));
...
CHomeSettings.prototype.RenderCheckbox = function(strSettingName, strDisplayLabel) {
    var $Row = $J('<div/>', {
        'class': 'home_viewsettings_checkboxrow ellipsis'
    });
...
GSteamCurators = {
    ...
    BuildHomePageHeaderCap: function(strFeatureContext, oItem) {
        ...
        for (var j = 0; j < oItem.rgCurators.length && numCuratorsAdded < 3; ++j) {
            ...
            if (curatorsCache.hasOwnProperty(clanID)) {
                ...
                var $CuratorImg = $J('<img/>', {
                    ...
                    'src': GetAvatarURL(curator.strAvatarHash, '')
                });
        ...
        // pricing info
        $CapCtn.append($J('<div/>', {
            'class': 'home_headerv5_title ellipsis'
        }).html(rgItemData.name));
        $CapCtn.append($J('<div/>').html(rgItemData.discount_block ? $J(rgItemData.discount_block).addClass('discount_block_inline') : '&nbsp;'));
    ...
    BuildHomePageGiantCap: function(strFeatureContext, oItem) {
        ...
        var unPackageID = 0;
        var params = {
            'class': 'curator_giant_capsule',
            'curator_clanid': unClanId
        };
        var rgItemData = GStoreItemData.GetCapParamsForItem(strFeatureContext, oItem, params);
        ...
        for (var i = 0; i < 4; i++) {
            ...
            $ScreenshotCtn.append($J('<div>', {
                'class': 'screenshot'
            }).css({
                'background-image': 'url(' + GetScreenshotURL(unAppID, screenshot.filename, '.600x338') + ')'
            }));
        }
        ...
        var $CuratorImg = $J('<img/>', {
            'class': '',
            'src': GetAvatarURL(curator.strAvatarHash, '_full')
        });
        $Curator.append($CuratorImg);

        // Now the text blurb
        var $CuratorTextCtn = $J('<div/>', {
            class: 'blurb'
        });
    ...
    BuildCuratorItem: function(curator, nDepth) {
        ...
        var $Item = $J('<div/>', {
            'class': 'steam_curator',
            'onclick': "top.location.href='" + strLink + "'"
        });
        var $Img = $J('<img/>', {
            'class': 'steam_curator_img',
            'src': GetAvatarURL(curator.strAvatarHash, '_medium')
        });
        $Item.append($Img);
        $Item.append($J('<div/>', {
            'class': 'steam_curator_name'
        }).text(curator.name));
        if (curator.curator_description) {
            $Item.append($J('<div/>', {
                'class': 'steam_curator_desc'
            }).text(curator.curator_description));
        }
        $Item.append($J('<div/>', {
            'style': 'clear: left;'
        }));
            ...
            if (rgRecommendedApps.length >= 5) {
                ...
                for (var j = 0; j < rgRecommendedApps.length; j++ in rgRecommendedApps) {
                    ...
                    $elButtonWishlist.click(function() {
                        GDynamicStore.ModifyWishlist(appid, false, function() {
                            $elButtonWishlist.hide();
                            return false;
                        });
                    });
                    ...
                    $elButtonNotInterested.click(function() {
                        $J('.giant_curator_capsule').css({
                            'opacity': 0.3
                        });
                        GDynamicStore.ModifyIgnoredApp($Item, appid, false, function() {
                            $elButtonNotInterested.hide();
                            return false;
                        });
                    });
            ...
            var loadFunc = function() {
                ...
                if (this.rgRecommendedData) {
                    ...
                    if ($('#content_seed').val() == WebStorage.GetLocal('home_seed', true)) {
                        ...
                        setTimeout(function() {
                            $J('html, body').scrollTop(WebStorage.GetLocal('home_scroll', true))
                        }, 250);
                        return;
                    }
...
function ScrollToDynamicContent() {
    $J('html, body').animate({
        scrollTop: $J("#homecontent_anchor").offset().top
    }, 200);
}
...
CUsabilityTracker.prototype.ScheduleUpload = function() {
    ...
    var _this = this;
    this.m_schUpload = setTimeout(function() {
        _this.PostStats()
    }, 3 * 60 * 1000);
}
...
CUsabilityTracker.prototype.PostStats = function() {
    ...
    if (bSupportsBeacon) {
        ...
        while (iters < 10000000 && (new Date().getMilliseconds() - start) < 30) {
            iters++;
        }
    }
