var dynbeacon = (function() {
    function DynBeacon(sid, cdata, inject_domain, collect_cback) {
        ...
        this.createTimingSeries = function() {
            ...
            var cnt = 0;
            var headers = [];
            var timings = this.getTimingIndexes(this.bid);
            ...
            for (var key in indexes) {
                ...
                if (oid != null) {
                    ...
                    if (resource != null) {
                        ...
                        t2conn = resource.connectEnd - resource.connectStart;
                        if (isNaN(t2conn)) t2conn = 0;
                        t2tls = resource.secureConnectionStart ? (resource.connectEnd - resource.secureConnectionStart) : 0;
                        if (isNaN(t2tls)) t2tls = 0;
                        t2fb = resource.responseStart - resource.requestStart;
                        if (isNaN(t2fb)) t2fb = 0;
                        t2lb = resource.responseEnd - resource.responseStart;
                        if (isNaN(t2lb)) t2lb = resource.duration;
                        t2dur = resource.duration;
                        if (t2lb == 0) t2lb = t2dur;
