...
(function($, Clipboard) {
    ...
    Drupal.behaviors.longReadShareLinksSticky = {
        attach: function(context, settings) {
            var container;
            if ((container = $('.pane-eff-share-blog.desktop:visible .share-links', context)[0])) {
                var top;
                var height;
                var bottom;

                var topMargin = parseFloat($(container).css('margin-top'));
                var bottomMargin = topMargin + parseFloat($('#footer').css('margin-top'));

                var reset = function() {
                    top = $('#main-content').offset().top - topMargin;
                    height = container.getBoundingClientRect().height;
                    bottom = $('#main-content').offset().top + $('#main-content').height();
                };

                var setposition = function(e, reset) {
                    requestAnimationFrame(function() {
                        ...
                        if (window.pageYOffset + height + bottomMargin > bottom) {
                            $(container).css('top', bottom - height - bottomMargin);
    ...
    Drupal.behaviors.longReadShareLinksCopy = {
        attach: function(context, settings) {
            if (Clipboard.isSupported()) {
                ...
                $('.share-clipboard', context).click(function(e) {
                    e.preventDefault();
                });
                ...
                $.post(e.target.action, formData)
                    .done(function(data) {
                        if (data.status) {
                            ...
                            if (settings.effPiwikUrl) {
                                $.post(settings.effPiwikUrl
        ...
        attach: function(context, settings) {
            ...
            $('.large-table', context).each(function(tableIndex) {
                ...
                if (currentTable.children.length > 0) {
                    columns.push({
                        targets: [0],
                        className: 'details details-closed',
                        orderable: false
                    });
