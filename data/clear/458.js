var headerNavigation = {
        submMenuClassName: "has-submenu",
        mobileMenuExists: false,
        escapeKeyCode: 27,
        _toggleMainMenu: function(event) {
            var currentOpenNavMenus = document.getElementsByClassName("has-submenu open"),
                findParentClassNode = this._findParentClassNode(this.submMenuClassName, event.target);
            if (!this._hasSubMenu(event)) {
                return
            }
            event.preventDefault();
            event.stopPropagation();
            if (currentOpenNavMenus.length > 0) {
                if (currentOpenNavMenus[0].classList.contains("toggle-mobile-overlay")) {
                    this.bodySelector.classList.remove("mobile-nav-open")
                }
                if (!currentOpenNavMenus[0].isSameNode(findParentClassNode)) {
                    currentOpenNavMenus[0].classList.remove("open")
                }
            }
            findParentClassNode.classList.toggle("open");
            if (document.getElementsByClassName("toggle-mobile-overlay open").length) {
                this.bodySelector.classList.add("mobile-nav-open")
            }
        },
        _hideAllMenus: function() {
            ...
            this.menuLinks.forEach(function(elem) {
                var parentNode = this._findParentClassNode(this.submMenuClassName, elem);
                if (parentNode) {
        ...
        _toggleMobileMenu: function() {
            this._hideAllMenus();
            this.bodySelector.classList.toggle("mobile-nav-open");
            this.mobileBtn.classList.toggle("open");
            this.mobileMenu.classList.toggle("open");
            this._hideAllMenus()
        },
        _isMenuItem: function(event) {
            return event.target.classList.contains("hd")
        },
        _hasSubMenu: function(event) {
            return this._findParentClassNode(this.submMenuClassName, event.target)
        },
        _isMobileMenuVisible: function() {
            var btnStyles = window.getComputedStyle(this.mobileBtn, null);
            return btnStyles.getPropertyValue("display") !== "none"
        },
        hasMobileMenu: function() {
            return this.mobileBtn && this.mobileOverlay
        },
        _findParentClassNode: function(parentClassName, child) {
            var testObj = child.parentNode,
                testClassName = testObj.getAttribute("class"),
                count = 1,
                regex = new RegExp(parentClassName, "ig");
            while (!regex.test(testClassName)) {
                if (/nav/gi.test(testClassName)) {
                    testObj = false;
                    break
                }
        ...
        handleClick: function(event) {
            if (this._isMenuItem(event)) {
                this._toggleMainMenu(event)
            } else {
                this._hideAllMenus()
            }
        },
        ...
        init: function() {
            this.menuLinks = [].slice.call(document.querySelectorAll("." + this.submMenuClassName + " a.hd"));
            this.bodySelector = document.querySelector("body");
            this.mobileBtn = document.querySelector(".hamburger-menu");
            this.mobileMenu = document.getElementById("mobileMenu");
            this.mobileOverlay = document.getElementById("mobileMenuOverlay");
            this.bodySelector.addEventListener("click", this.handleClick.bind(this), false);
            this.mobileMenu.addEventListener("click", this.handleClick.bind(this), false);
            document.addEventListener("keyup", this.handleKeyup.bind(this), false);
            if (this.hasMobileMenu()) {
                this.mobileBtn.addEventListener("click", this.handleMobileClick.bind(this), false);
                this.mobileOverlay.addEventListener("click", this._toggleMobileMenu.bind(this), false)
            }
        }
    },
    classListPolyfill = function() {
...
function addScroll() {
    addClass(".base-header.anon-header .nav li a", "scroll-li");
    addClass(".anon-header .user-area a", "scroll-buttons");
    addClass(".base-header.anon-header", "scroll-sticky");
    addClass(".base-header.anon-header .responsive-logo", "scroll-logo");
    addClass(".anon-header .hamburger-line", "scroll")
}

function addStatic() {
    addClass(".anon-header .user-area a", "static-buttons");
    addClass(".base-header.anon-header .responsive-logo", "static-logo");
    addClass(".base-header.anon-header .nav li a", "static-li");
    initializeExperiments();
    addClass(".base-header.anon-header", "static-background");
    addClass(".anon-header .hamburger-line", "static")
}

function removeStatic() {
    removeClass(".anon-header .hamburger-line", "static-logo", "static");
    removeClass(".base-header.anon-header .responsive-logo", "static-logo");
    removeClass(".anon-header .user-area a", "static-buttons");
    removeClass(".base-header.anon-header .nav li a", "static-li")
}

function removeScroll() {
    initializeExperiments();
    removeClass(".anon-header .hamburger-line", "scroll");
    removeClass(".base-header.anon-header .responsive-logo", "scroll-logo");
    removeClass(".base-header.anon-header", "scroll-sticky");
    removeClass(".anon-header .user-area a", "scroll-buttons");
    removeClass(".base-header.anon-header .nav li a", "scroll-li")
}

function setSignUpButton() {
    var el = document.getElementsByClassName("sign-up-cta-copy");
    var signUpText = el[0].innerText;
    var signInButton = document.querySelector(".anon-header .sign-in");
    if (signInButton) {
        signInButton.setAttribute("href", "/user/sign-up/");
        signInButton.innerText = signUpText
    }
}

function setSignInButton() {
    var el = document.getElementsByClassName("sign-in-cta-copy");
    var signInText = el[0].innerText;
    var signInButton = document.querySelector(".anon-header .sign-in");
    if (signInButton) {
        signInButton.setAttribute("href", "/user/sign-in/");
...
function getExperimentAssignment() {
    initializeExperiments();
    var el = document.getElementsByClassName("experiment-info");
    for (var i = 0; i < el.length; i++) {
        var _assignment = el[i].getAttribute("data-assignment"),
            _actionType = el[i].getAttribute("data-action-type");
        if (_assignment && _actionType) {
            window.experiments[_actionType] = _assignment
        }
    }
}

function bindArrowElEventListener(arrow_el) {
    if (!document.querySelector(".scroll-target")) {
        return
    }
    var scroll_target = document.querySelector(".scroll-target"),
        scroll_y = scroll_target.getBoundingClientRect().top - getHeaderHeight(),
        event_listener = function() {
            slideTo(0, scroll_y, 600)
        };
    if (!!!arrow_el.getAttribute("arrow_event_handler")) {
        arrow_el.addEventListener("click", event_listener);
        arrow_el.setAttribute("arrow_event_handler", true)
    }
}

function updateArrowElement() {
    if (window.showScrollArrow) {
        var arrow_el = document.getElementById("hero-arrow"),
            scroll_pos = window.pageYOffset;
        if (scroll_pos > 10) {
            arrow_el.style.visibility = "hidden"
        } else {
            arrow_el.style.visibility = "visible";
            bindArrowElEventListener(arrow_el)
        }
    }
}
...
function bindEvents() {
    initializeExperiments();
    window.addEventListener("scroll", checkScrollPosAndUpdateStyles);
    window.addEventListener("scroll", updateArrowElement)
}

function slideTo(x, y, time, decelRate, interval) {
    if (!decelRate) decelRate = 1;
...
function slideTo_h(x, y, time, decelRate, interval, dateTime) {
    ...
    setTimeout(function() {
        slideTo_h(x, y, time - interval * decelRate, decelRate, interval, dateTime + interval)
...
var checkScrollPosAndUpdateStyles = function() {
...
function getHeaderHeight() {
    ...
    return $(".base-header").height()
}
document.addEventListener("DOMContentLoaded", function(event) {
    if (!document.getElementById("dd-my-account")) {
        getExperimentAssignment();
        staticStyle();
        checkScrollPosAndUpdateStyles()
    }
    updateArrowElement();
    bindEvents()
});
var carousel = {
    _CAROUSEL_LIST_SELECTOR: ".slide",
    leftBtn: null,
    rightBtn: null,
    init: function() {
        var self = this;
        self.$carousel_list = document.querySelectorAll(this._CAROUSEL_LIST_SELECTOR);
        self.watchSwipeBox();
        self.leftBtn = document.querySelector(".leftbtn");
        self.rightBtn = document.querySelector(".rightbtn");
        self.leftBtn.addEventListener("click", function(e) {
            e.preventDefault();
            self.handleLeftClick()
        });
        self.rightBtn.addEventListener("click", function(e) {
            e.preventDefault();
            self.handleRightClick()
        });
        var body = document.querySelector("body");
        body.delegateEventListener("mouseover", ".slide:not(.s4)", function(e) {
            self.setCSS(".s4", "transform", "scale(1.0)")
        });
        body.delegateEventListener("mouseout", ".slide:not(.s4)", function(e) {
            self.setCSS(".s4", "transform", "scale(1.1)")
        });
        body.delegateEventListener("click", ".s5", function(e) {
            self.leftBtn.click()
        });
        body.delegateEventListener("click", ".s6", function(e) {
            self.leftBtn.click();
            setTimeout(function() {
                self.leftBtn.click()
            }, 50)
        });
        body.delegateEventListener("click", ".s3", function(e) {
            self.rightBtn.click()
        });
        body.delegateEventListener("click", ".s2", function(e) {
            self.rightBtn.click();
            setTimeout(function() {
                self.rightBtn.click()
            }, 50)
    ...
    handleLeftClick: function() {
        for (i = 0; i < this.$carousel_list.length; i++) {
            this.$carousel_list[i].style.transform = null;
            if (this.$carousel_list[i].classList.contains("s1")) {
                this.$carousel_list[i].classList.remove("s1");
                this.$carousel_list[i].classList.add("s7-left")
            } else if (this.$carousel_list[i].classList.contains("s1-right")) {
                this.$carousel_list[i].classList.remove("s1-right");
                this.$carousel_list[i].classList.add("s7-left")
            } else if (this.$carousel_list[i].classList.contains("s2")) {
                this.$carousel_list[i].classList.remove("s2");
                this.$carousel_list[i].classList.add("s1")
            } else if (this.$carousel_list[i].classList.contains("s3")) {
                this.$carousel_list[i].classList.remove("s3");
                this.$carousel_list[i].classList.add("s2")
            } else if (this.$carousel_list[i].classList.contains("s4")) {
                this.$carousel_list[i].classList.remove("s4");
                this.$carousel_list[i].classList.add("s3")
            } else if (this.$carousel_list[i].classList.contains("s5")) {
                this.$carousel_list[i].classList.remove("s5");
                this.$carousel_list[i].classList.add("s4")
            } else if (this.$carousel_list[i].classList.contains("s6")) {
                this.$carousel_list[i].classList.remove("s6");
                this.$carousel_list[i].classList.add("s5")
            } else if (this.$carousel_list[i].classList.contains("s7")) {
                this.$carousel_list[i].classList.remove("s7");
                this.$carousel_list[i].classList.add("s6")
            } else if (this.$carousel_list[i].classList.contains("s7-left")) {
                this.$carousel_list[i].classList.remove("s7-left");
                this.$carousel_list[i].classList.add("s6")
            }
        }
    },
    handleRightClick: function() {
        for (i = 0; i < this.$carousel_list.length; i++) {
            this.$carousel_list[i].style.transform = null;
            if (this.$carousel_list[i].classList.contains("s1-right")) {
                this.$carousel_list[i].classList.remove("s1-right");
                this.$carousel_list[i].classList.add("s2")
            } else if (this.$carousel_list[i].classList.contains("s1")) {
                this.$carousel_list[i].classList.remove("s1");
                this.$carousel_list[i].classList.add("s2")
            } else if (this.$carousel_list[i].classList.contains("s2")) {
                this.$carousel_list[i].classList.remove("s2");
                ...
                this.$carousel_list[i].classList.remove("s6");
                this.$carousel_list[i].classList.add("s7")
            } else if (this.$carousel_list[i].classList.contains("s7-left")) {
                this.$carousel_list[i].classList.remove("s7-left");
                this.$carousel_list[i].classList.add("s1-right")
            } else if (this.$carousel_list[i].classList.contains("s7")) {
                this.$carousel_list[i].classList.remove("s7");
                this.$carousel_list[i].classList.add("s1-right")
            }
    ...
    watchSwipeBox: function() {
        var self = this;
        var els = document.querySelectorAll(".slide");
        swipeHandler.init(els, function(direction) {
            switch (direction) {
                case "left":
                    self.rightBtn.click();
                    break;
...
document.addEventListener("DOMContentLoaded", function() {
    var carouselContainer = document.querySelector(".motion");
    if (carouselContainer) {
        carousel.init()
    }
});
var swipeHandler = function() {
    var startX, startY, threshold = 20,
        isMoving = false,
        callback = null;
    ...
    function onTouchMove(e) {
        ...
        if (isMoving) {
            ...
            if (Math.abs(dx) * ratio >= threshold) {
                dir = dx > 0 ? "left" : "right"
            } else if (Math.abs(dy) * ratio >= threshold) {
                dir = dy > 0 ? "up" : "down"
            }
            if (dir) {
    ...
    return {
        init: function(els, callbackfunc) {
...
(function() {
    var hotjarSetup = {
        _EXPERIMENT_INFO: "#experiment-info",
        init: function() {
            this.$experimentInfo = document.querySelector(this._EXPERIMENT_INFO);
            this.triggerHotjar()
        },
        triggerHotjar: function() {
            var experimentInfo = this.$experimentInfo;
            var hotjarEnabled = typeof hj === "function";
            if (experimentInfo && hotjarEnabled) {
                var experiment_variation = experimentInfo.getAttribute("data-assignment");
                hj("trigger", experiment_variation)
            }
        }
    };
    document.addEventListener("DOMContentLoaded", function() {
...
$(function() {
    ...
    setTimeout(function() {
        $("#item-2").slideDown(1e3)
    }, 3e3);
    setTimeout(function() {
        $("#item-2").slideUp(1e3)
    }, 4e3);
    setTimeout(function() {
        $("#item-3").slideDown(1e3)
    }, 5e3);
    setTimeout(function() {
        $("#item-3").slideUp(1e3)
    }, 6e3)
