...
function ShowConfirmDialog(strTitle, strDescription, strOKButton, strCancelButton, strSecondaryActionButton) {
    ...
    var fnSecondary = function() {
        deferred.resolve('SECONDARY');
    };
    var fnCancel = function(bWasCancelButton) {
        deferred.reject(bWasCancelButton);
    };
    ...
    if (strSecondaryActionButton) {
        var $SecondaryActionButton = _BuildDialogButton(strSecondaryActionButton, false, {
            strClassName: ' btn_darkblue_white_innerfade btn_medium'
        });
...
function ShowAlertDialog(strTitle, strDescription, strOKButton) {
    ...
    var deferred = new jQuery.Deferred();
    var fnOK = function(bWasCancelButton) {
        deferred.resolve(bWasCancelButton);
    };
...
function ShowPromptDialog(strTitle, strDescription, strOKButton, strCancelButton, rgModalParams) {
    ...
    $Body.append($J('<div/>', {
        'class': 'newmodal_prompt_description'
    }).append(strDescription));
    $Body.append($J('<div/>', {
        'class': 'newmodal_prompt_input gray_bevel for_text_input fullwidth'
    }).append($Input));
...
function ShowPromptWithTextAreaDialog(strTitle, strDescription, strOKButton, strCancelButton, textAreaMaxLength) {
    ...
    var $TextArea = $J('<textarea/>', {
        'class': 'newmodal_prompt_textarea'
    });
    ...
    $Body.append($J('<div/>', {
        'class': 'newmodal_prompt_with_textarea gray_bevel fullwidth '
    }).append($TextArea));
...
function ShowEditablePrompt(strTitle, obj, onOk, onCancel) {
    ...
    $OKButton.click(function() {
        deferredAction.resolve($TextArea.val());
    });
    ...
    deferredAction.always(function() {
        Modal.Dismiss();
    });
...
function ShowBlockingWaitDialog(strTitle, strDescription) {
    ...
    var container = $J('<div/>', {
        'class': 'waiting_dialog_container'
    });
    var throbber = $J('<div/>', {
...
function _BuildDialog(strTitle, strDescription, rgButtons, fnOnCancel, rgModalParams) {
    ...
    var $CloseButton = $J('<div/>', {
        'class': 'newmodal_close'
    });
    var $Header = ($J('<div/>', {
        'class': 'newmodal_header'
    }));
    var $TopBar = ($J('<div/>', {
        'class': 'modal_top_bar'
    }));
    $Header.append($CloseButton).append($J('<div/>', {
        'class': 'title_text'
    }).text(strTitle));
    ...
    if (Modal.m_fnBackgroundClick) {
        $Dialog.click(function(e) {
            if (e.target == this) Modal.m_fnBackgroundClick();
        });
    }
...
function CModal($Content, rgParams) {
    ...
    var _modal = this;
    this.m_fnBackgroundClick = function() {
        if (_modal.BIsActiveModal() && _modal.m_bDismissOnBackgroundClick) {
            _modal.Dismiss();
        }
    };
    this.m_fnOnEscapeKeyPress = function(event) {
        if (_modal.BIsActiveModal() && event.which == 27) _modal.m_fnBackgroundClick();
    };
...
CModal.prototype.AdjustSizing = function(duration) {
    ...
    if (duration) {
        this.m_$Content.animate({
            'max-width': nMaxWidth,
            left: nLeft,
            top: nTop
        }, duration);
        this.m_$StandardContent.animate({
            'max-height': nMaxHeight
        }, duration);
    } else {
...
CModal.ShowModalBackground = function() {
    if (!CModal.s_$Background) {
        CModal.s_$Background = $J('<div/>', {
            'class': 'newmodal_background'
        });
...
function InitMiniprofileHovers() {
    var $Hover = $J('<div/>', {
        'class': 'miniprofile_hover'
    });
    var $HoverContent = $J('<div/>', {
        'class': 'miniprofile_hover_inner shadow_content'
    });

    var $HoverArrowLeft = $J('<div/>', {
        'class': 'hover_arrow left miniprofile_arrow'
    });
    $HoverArrowLeft.append('<div class="miniprofile_arrow_inner"></div>');
    var $HoverArrowRight = $J('<div/>', {
        'class': 'hover_arrow right miniprofile_arrow'
    });
    ...
    var fnDataFactory = function(key) {
        return new CDelayedAJAXData('miniprofile/' + key, MINIPROFILE_DELAY_BEFORE_AJAX);
    };
...
function BindAJAXHovers($Hover, $HoverContent, oParams) {
    ...
    var fnReadKey = function($Element) {
        return $Element.data(strDataName);
    };
...
function InitEmoticonHovers() {
    var $Hover = $J('<div/>', {
        'class': 'emoticon_hover'
    });
    var $HoverContent = $J('<div/>', {
        'class': 'emoticon_hover_content'
    });
    $Hover.append($HoverContent, $J('<div/>', {
        'class': 'hover_arrow left emoticon_hover_arrow'
    }), $J('<div/>', {
        'class': 'hover_arrow right emoticon_hover_arrow'
    }));
    $Hover.hide();
...
function DynamicLink_PlayYouTubeVideoInline(elem, videoid) {
    ...
    var wrapper = $J('<div/>', {
        'class': 'dynamiclink_youtubeviewvideo'
    });
    var embed_wrapper = $J('<div/>', {
        'class': 'dynamiclink_youtubeviewvideoembedded',
        'frameborder': '0'
    });
    var iframeContent = $J('<iframe/>', {
        'frameborder': '0',
        src: youtubeurl,
        'allowfullscreen': ''
    });
...
function DynamicLink_ShowSketchfabModelInline(elem, details) {
    ...
    var el = $J(elem);
    var wrapper = $J('<div/>', {
        'class': 'dynamiclink_sketchfabmodelembedded',
        'frameborder': '0'
    });
    var iframeContent = $J('<iframe/>', {
        'frameborder': '0',
...
CScrollOffsetWatcher.RegisterWatcher = function(Watcher) {
    ...
    if (!CScrollOffsetWatcher.m_nTimeoutInitialLoad)
        CScrollOffsetWatcher.m_nTimeoutInitialLoad = window.setTimeout(function() {
...
function LoadImageGroupOnScroll(elTarget, strGroup) {
    ...
    if ($Element.length)
        new CScrollOffsetWatcher($Element, function() {
            LoadDelayedImages(strGroup);
        });
}
...
function CAjaxInfiniteScrollingControls(rgSearchData, url) {
    ...
    $J(document).scroll(function() {
        return scrollFunc.apply(thisControl)
    });

    window.addEventListener('beforeunload', function(event) {
        thisControl.OnUnload(event);
    });
...
CAjaxPagingControls.prototype.AddPageLink = function(elPageLinks, iPage) {
    var el = new Element('span', {
        'class': (this.m_strClassPrefix != "" ? this.m_strClassPrefix : this.m_strElementPrefix) + '_paging_pagelink'
    });
    el.update((iPage + 1) + ' ');
...
function CScrollSlider($Scroll, $Container, $Grabber, args) {
    ...
    this.m_Slider = new CSlider($Slider.length ? $Slider : $Container, $Grabber, {
        fnOnChange: $J.proxy(this.OnSliderChange, this)
    });
...
window.VScrollbar = function(eleTarget, eleHandle, direction) {
    ...
    if (direction == k_EScrollbarDirectionVertical) {
        ...
        this.m_eleTarget.first().css({
            'white-space': 'nowrap'
        });
    }

    // Set up some CSS properties we need
    this.m_eleHandle.css({
        position: 'absolute'
    });
    if (this.m_eleHandle.parent().css('position') == 'static')
        this.m_eleHandle.parent().css({
            position: 'relative'
        }); // Needs to be relative or absolute, only set it if we didn't do it in CSS
    this.m_eleTarget.css({
        position: 'relative',
        float: 'left'
    });
...
function CAutoComplete(elInput, fnSearch, fnOnChange) {
    ...
    var _this = this;
    this.m_$Input.on('keydown.autocomplete', function(event) {
        _this.OnInputKeyDown(event);
    });
    this.m_$Input.on('keyup.autocomplete', function(event) {
        _this.OnInputKeyUp(event);
    });
    this.m_$Input.on('blur.autocomplete', function(event) {
        _this.OnInputBlur(event);
    });
    this.m_$Input.on('change input paste', function(event) {
        _this.OnInputChange(event);
    });

    this.m_$Popup.on('mousedown.autocomplete', function(event) {
        _this.m_$Popup.data('mousedown', true);
    });
    this.m_$Popup.on('mouseup.autocomplete', function(event) {
        _this.m_$Popup.data('mousedown', false);
    });
}
...
CAutoComplete.prototype.OnInputChange = function(event) {
    ...
    if (this.m_hSearchTimeout == 0) {
        ...
        var fnDoSearch = function() {
            ...
            _this.m_fnSearch(_this.m_$Input, strSearch, function($Contents) {
                _this.SetPopupContents($Contents);
            });
        };
...
function FixedElementOnScrollWrapper(elemID, fixedOffsetTop, bScrollWithPageIfTooTall, docHeightOffset, params) {
    ...
    this.homePosn = {
        x: this.fixedElement.cumulativeOffset()[0],
        y: this.fixedElement.cumulativeOffset()[1]
    };
    this.fixedClass = params.fixedClass;


    this.$FixedElementPadding = $J('<div/>', {
        'class': 'FixedElementOnScrollWrapper_padding',
        'id': 'ScrollWrapperPadding_' + elemID
    }).hide();
    this.fixedElement.insert({
        before: this.$FixedElementPadding[0]
    });

    var _this = this;
    $J(window).on('scroll.FixedElementOnScrollWrapper hashchange.FixedElementOnScrollWrapper', function() {
        _this.handleScroll()
    });
    $J(window).on('resize.FixedElementOnScrollWrapper', function() {
        _this.handleScroll(true /* force recalc */ )
    });
    this.handleScroll();
}
...
CTextInputSuggest.prototype.Init = function($InputElement, fnSuggestForTerm, fnOnSuggest, strCssClass) {
    ...
    this.m_$SuggestionsCtn = $J('<div/>', {
        'class': strCssClass,
        style: 'display: none;'
    });
    this.m_$Suggestions = $J('<div/>', {
    ...
    var _this = this;
    this.m_$Input.on('keyup.CTextInputSuggest click.CTextInputSuggest', function(event) {
        _this.OnTextChanged(event)
    });
    this.m_$Input.on('paste.CTextInputSuggest cut.CTextInputSuggest', function() {
        window.setTimeout(function() {
            _this.OnTextChanged()
        }, 1);
    });
    this.m_$Input.on('keydown.CTextInputSuggest', function(event) {
        _this.OnKeyDown(event)
    });
    this.m_$Input.on('focus.CTextInputSuggest', function() {
        _this.ShowSuggestions()
    });
    this.m_$Input.on('blur.CTextInputSuggest', function() {
...
CTextInputSuggest.prototype.SetSuggestions = function(rgSuggestions) {
    ...
    if (rgSuggestions && rgSuggestions.length) {
        ...
        for (var i = 0; i < rgSuggestions.length; i++) {
            var $Suggestion = $J('<div/>', {
                'class': 'suggestion_item popup_menu_item'
            });
...
var CAjaxSubPageController = function(elTarget, strBaseURL, strInstanceId, strDefaultLocation) {
    ...
    this.rgOriginalEvent = {
        'html': this.elTarget.innerHTML,
    ...
    var _this = this;
    setTimeout(function() {
        $J(_this.elTarget).children().trigger('load');
    }, 1);
...
CAjaxSubPageController.prototype.Navigate = function(strLocation, strPageTitle, event) {
    ...
    $J.ajax({
        ...
        dataType: "html",
        cache: true,
        /* Let the browser decide caching rules */
        data: {
            'ajax': 1
        }
...
function BindTooltips(selector, rgOptions) {
    ...
    if ($TextTooltips.length)
        $TextTooltips.v_tooltip({
            'tooltipClass': rgOptions.tooltipCSSClass,
...
function ViewTitlesWithDescriptors(descid) {
    ...
    $J.get('https://store.steampowered.com/search/results/', {
        ...
        var content = $J("<div>");
        content.append($J("<div>", {
            class: 'content_descriptors_examples_desc',
            text: response.desc
        }));
        ...
        if (response.items.length != 0) {
            for (var i = 0; i < response.items.length && i < 10; ++i) {
                ...
                var elem = $J("<div>", {
                    class: 'content_descriptors_example_app'
                });
                elem.append($J("<img>", {
                    class: 'app_logo',
                    src: item.logo
