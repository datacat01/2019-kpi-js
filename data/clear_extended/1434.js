    ...
    if (tabStart[tab] > tabMax[tab]) {
        ...
        new Ajax.Updater(
            ...
            'https://store.steampowered.com/search/tab', {
                ...
                method: 'get',
                insertion: 'bottom',
                onComplete: TabCompletionClosure(tab, delta, max)
            });
...
function RollTab(tab, delta) {
    if ($('tab_' + tab + '_items').hasClassName('twenty_smallcap_page_items')) {
        ...
        if (delta > 0)
            xdiff = -xdiff;
        new Effect.Move($('tab_' + tab + '_items'), {
            x: xdiff,
            afterFinish: TabScrollFinishClosure(tab, delta)
        });
    } else {
        //standard tab
    ...
    if (elem.curPos > elem.maxLoaded) {
        ...
        new Ajax.Updater(
            ...
            'https://store.steampowered.com/search/smallcapscroll', {
                ...
                method: 'get',
                insertion: 'bottom',
                onComplete: UpdateSmallCapControl.bind(window, targetid, delta, pageSize, totalCount)
            });
    } else {
...
function ScrollCarouselSmallCaps(targetid, delta, pageSize, totalCount) {
    ...
    elem.effect = new Effect.Move(elem, {
        x: -808 * delta,
        afterFinish: cb.bind(this, targetid),
        duration: 0.4
    });
...
function GameHover(elem, event, divHover, rgHoverData) {
    ...
    if (g_HoverState.hiding && $Hover.is(':visible') && g_HoverState.target == $Elem[0]) {
        ...
        var fnComputeHoverDelay = bNewHoverSpeed ?
            function() {
                return Math.max(400 - (new Date().getTime() - nStartHoverTime), 200);
            } :
            function() {
                return 200;
            };
...
function ShowGameHover(elem, divHover, targetContent, params, speed) {
    ...
    $Hover.offset({
        top: nHoverPositionTop,
        left: nHoverPositionLeft
    });
    ...
    if (!link.href.match(/^javascript/)) {
        ...
        if (navinfo) {
            ...
            $J(link).click(function(event) {
                InstrumentedLinkOnClick(event, link);
            });
        } else {
...
function expandTXItem(item) {
    ...
    if (blurb.visible())
        Effect.BlindUp(blurb, {
            duration: 0.25
        });
    else
        Effect.BlindDown(blurb, {
            duration: 0.25
        })
}
...
var CCountdownManager = {
    ...
    refreshOnUserAction: function() {
        ...
        if (!this.bReadyForRefresh && Math.round(new Date().getTime() / 1000) - this.tsInit >= 5) {
            var _this = this;
            $J(document).on('focus mousemove', function() {
                _this.doRefresh();
            });
...
function EnableSearchSuggestions(elemTerm, navcontext, cc, l, rgUserPreferences, strPackageXMLVersion, elemSuggestionCtn, elemSuggestions) {
    ...
    $Term.on('keydown', function(event) {
        SearchSuggestOnKeyDown(event, $Term, $SuggestionsCtn, $Suggestions);
    });
    ...
    if (value) {
        ...
        $J.get('https://store.steampowered.com/search/suggest', parameters).done(function(html) {
            ...
            $Suggestions.find('a.match').each(function() {
                var el = this;
                $J(el).on('mouseover', function(event) {
                    SearchSuggestOnMouseOver(event, $J(el));
                });
            });
            ...
            if (rgYourPopularTags.length > 0) {
                ...
                for (var i = 0; i < rgYourPopularTags.length && i < 5; i++) {
                    ...
                    var $Link = $J('<a/>', {
                        ...
                        'href': 'https://store.steampowered.com/tag/en/' + encodeURIComponent(tag.name)
                    });
                    $Link.text(tag.name);
                    $Element.append($Link);
                }
...
function AddFreeLicense(subid, strDisplayName) {
    ...
    for (var i = 0; i < subids.length; i++) {
        posts.push($J.post('https://store.steampowered.com/checkout/addfreelicense/' + subids[i], {
            ajax: true,
            sessionid: g_sessionID
        }));
    }
...
function ChangeLanguage(strTargetLanguage, bStayOnPage) {
    ...
    $J.post('https://store.steampowered.com/account/setlanguage/', {
            language: strTargetLanguage,
            sessionid: g_sessionID
        })
        .done(function() {
            if (bStayOnPage)
...
function Logout() {
    var $Form = $J('<form/>', {
        'action': 'https://store.steampowered.com/logout/',
        'method': 'POST'
    });
    $Form.append($J('<input/>', {
        'type': 'hidden',
        ...
        if (allButtons.length > 0) {
            ...
            if (button != null && button.length === 1 && typeof button.offset == 'function') {
                ...
                if (formSelector.length === 1) {
                    ...
                    jQuery('<input type="hidden">').attr({
                        name: 'x_top',
                        'value': buttonOffset.top
                    }).appendTo(formSelector);
                    jQuery('<input type="hidden">').attr({
                        name: 'x_left',
                        'value': buttonOffset.left
                    }).appendTo(formSelector);
                    jQuery('<input type="hidden">').attr({
                        name: 'x_window_height',
                        'value': height
                    }).appendTo(formSelector);
                    jQuery('<input type="hidden">').attr({
                        name: 'x_window_width',
...
function InitHorizontalAutoSliders() {
    $J('.store_horizontal_autoslider').each(function() {
        var $Scroll = $J(this);
        var $Wrapper = $Scroll.wrap($J('<div/>', {
            'class': 'store_horizontal_autoslider_ctn'
        })).parent();
        var $SliderCtn = $J('<div/>', {
            'class': 'slider_ctn store_autoslider'
        });
        var $SliderLeft = $J('<div/>', {
            'class': 'slider_left'
        }).append($J('<span/>'));
        var $SliderRight = $J('<div/>', {
            'class': 'slider_right'
        }).append($J('<span/>'));
        var $Slider = $J('<div/>', {
            'class': 'slider'
        ...
        var fnFixHeight = function() {
            $Wrapper.height($Scroll[0].clientHeight);
        };
    ...
    $J('.store_horizontal_minislider').each(function() {
        var $Scroll = $J(this);
        var $Wrapper = $Scroll.wrap($J('<div/>', {
            'class': 'store_horizontal_minislider_ctn'
        })).parent();
        var $SliderLeft = $J('<div/>', {
            'class': 'slider_left'
        }).append($J('<span/>'));
        ...
        $SliderLeft.click(function() {
            $Scroll.animate({
                scrollLeft: Math.max(0, $Scroll.scrollLeft() - $Wrapper.width())
            });
            fnShowHideButtons();
        });
        $SliderRight.click(function() {
            $Scroll.animate({
                scrollLeft: Math.min($Scroll[0].scrollWidth - $Scroll.width(), $Scroll.scrollLeft() + $Wrapper.width())
            });
            fnShowHideButtons();
        });
...
function PreloadImages(elElement) {
    $J(elElement).find("*[data-background-image-url]").each(function(i, j) {
        var $elTarget = $J(j);
        $elTarget.css({
            'background-image': 'url(' + $elTarget.data('background-image-url') + ')'
        });
    });
...
var CGenericCarousel = function($elContainer, nSpeed, fnOnFocus, fnOnBlur, fnClickThumb, bNoWrap) {
    ...
    $elContainer.bind('mouseover', function(e) {
        instance.fnMouseOver();
    });
    $elContainer.bind('mouseout', function(e) {
        instance.fnMouseOut();
    });
    // If we get a scroll event, and we're in respondive, hint all remaining images
    this.$elItems.parent().bind('scroll', function(e) {
        if (instance.bIsResponsive()) PreloadImages($elContainer);
    });
    ...
    this.$elArrowLeft.click(function(e) {
        instance.Advance(-1);
        e.preventDefault();
        return true;
    });
    this.$elArrowRight.click(function(e) {
        instance.Advance();
        e.preventDefault();
        return true;
    });
...
CGenericCarousel.prototype.ResponsiveAdvance = function(nNewIndex) {
    ...
    $J($elTarget).stop().animate({
        scrollLeft: nTargetScroll
    }, 800);
