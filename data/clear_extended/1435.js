...
GDynamicStore = {
    ...
    Init: function(accountid, bForceRefresh, strOS, preferences) {
        ...
        for (var i = 0; i < rgDesiredOSTypes.length; i++) {
            switch (rgDesiredOSTypes[i]) {
                ...
                default:
                case 'win':
                    GDynamicStore.s_bUserOnWindows = true;
                    break;
            }
        }
    ...
    DecorateDynamicItems: function($Selector, bForceRecalculate) {
        ...
        if (!GDynamicStore.m_bLoadComplete) {
            GDynamicStore.OnReady(function() {
                GDynamicStore.DecorateDynamicItems($Selector)
            });
            return;
        }
    ...
    UpdateDynamicBundleElements: function(Bundle, $El) {
        ...
        if (!Bundle.m_bIsCommercial && (!Bundle.m_rgBundleItems.length || (Bundle.m_bMustPurchaseAsSet && Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle))) {
            ...
            if (!Bundle.m_bContainsDiscountedPackage) {
                ...
                $DiscountBlocks.find('.discount_final_price').addClass('your_price').empty().append($J('<div/>', {
                    'class': 'your_price_label'
                }).text('Your Price:'), $J('<div/>').text(strFormattedFinalPrice));
            } else {
    ...
    BuildBundleDescription: function(Bundle, $Description) {
        if (Bundle.m_cUserItemsInBundle == 0) {
            ...
            $Description.parents('.dynamic_bundle_description').append($J('<div/>', {
                'class': 'ds_flag ds_completetheset'
        ...
        if (rgItemsWithCaps.length) {
            ...
            var $BundleContentsCtn = $J('<div/>', {
                'class': 'bundle_contents_preview'
            });
            if (bNeedToCollapse)
                $BundleContentsCtn.addClass('collapsed');

            var $BundleContentsPosition = $J('<div/>', {
                'class': 'bundle_contents_preview_position'
            });
            for (var i = 0; i < rgItemsWithCaps.length; i++) {
                var rgLinkParams = {
                    'class': 'bundle_contents_preview_item ds_collapse_flag'
                };
                ...
                var $Img = $J('<img/>', {
                    'src': Item.tiny_capsule,
                    'class': 'bundle_contents_preview_img'
                });
    ...
    PopulateRecommendedTagList: function() {
        ...
        for (var i = 0; i < GDynamicStore.s_rgRecommendedTags.length && i < 5; i++) {
            ...
            var $Link = $J('<a/>', {
                ...
                'href': 'https://store.steampowered.com/tags/en/' + encodeURIComponent(tag.name)
            });
            $Link.text(tag.name);
            $Element.append($Link);
        }
        ...
        if (GDynamicStore.s_rgCreatorsFollowed) {
            for (var clanid in rgAppCreatorRelationship) {
                if (rgAppCreatorRelationship.hasOwnProperty(clanid) && clanid in GDynamicStore.s_rgCreatorsFollowed) {
                    return {
                        'clanid': clanid,
                        'relationship': rgAppCreatorRelationship[clanid]
                    }
                }
            }
        }
    ...
    DisplayBundleSimulator: function(unBundleID) {
        if (!GDynamicStore.m_bLoadComplete) {
            GDynamicStore.OnReady(function() {
                GDynamicStore.DisplayBundleSimulator(unBundleID)
            });
            return;
        }
...
GStoreItemData = {
    ...
    rgAccountData: [],
    rgNavParams: {},
    fnFormatCurrency: function(nValueInCents) {
        return v_numberformat(nValueInCents / 100);
    },
    ...
    GetStoreItemDataForElement: function($el) {
        ...
        if (unBundleID && GStoreItemData.rgBundleData[unBundleID])
            return {
                bundleid: unBundleID,
                item: GStoreItemData.rgBundleData[unBundleID]
            };
        ...
        if (unPackageID && GStoreItemData.rgPackageData[unPackageID])
            return {
                packageid: unPackageID,
                item: GStoreItemData.rgPackageData[unPackageID]
            };
        ...
        if (unAppId && GStoreItemData.rgAppData[unAppId])
            return {
                appid: unAppId,
                item: GStoreItemData.rgAppData[unAppId]
            };
