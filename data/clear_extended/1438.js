function Cluster(args) {
    ...
    var _this = this;
    this.elScrollLeftBtn.on('click', function(event) {
        _this.scrollLeft(event);
    });
    this.elScrollRightBtn.on('click', function(event) {
        _this.scrollRight(event, false);
    });

    this.elScrollLeftBtn.on('touchend', function(event) {
        _this.mouseOut(event);
    });
    this.elScrollRightBtn.on('touchend', function(event) {
        _this.mouseOut(event);
    });

    this.elClusterArea.on('mouseenter', function(event) {
        _this.mouseOver(event);
    });
    this.elClusterArea.on('mouseleave', function(event) {
        _this.mouseOut(event);
    });
...
Cluster.prototype.initSlider = function() {
    ...
    this.slider = new CSlider(this.elSlider, this.elHandle, {
        ...
        value: 0,
        fnOnChange: function(value, bInDrag) {
            _this.sliderOnChange(value, bInDrag);
        }
    });
};
...
Cluster.prototype.scrollToOffset = function(nXOffset, nDuration, fnOnComplete) {
        if (nDuration) {
            this.elScrollArea.parent().animate({
                scrollLeft: nXOffset
            }, nDuration, null, fnOnComplete);
        } else {
...
Cluster.BuildClusterCapsule = function(unAppID, unPackageID, strStatus, strFeature, nDepth, rgItemInfo) {
    ...
    $CapCtn.append($J('<div/>', {
        ...
        .append($J('<div/>', {
                'class': 'main_cap_content'
            })
            .append($J('<div/>', {
                'class': 'main_cap_platform_area platform_area'
            }).html(GStoreItemData.BuildSupportedPlatformIcon(rgItemData)))
            .append($J('<div/>', {
                    'class': 'main_cap_status ellipsis'
                }).text(strStatus)
