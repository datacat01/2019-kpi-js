...
(function($) {
    ...
    Drupal.behaviors.effPiwikContentTracking = {
        attach: function(context, settings) {

            /**
             * Helper function to build piwik.php URL.
             *
             * @param {string} url
             *   The Piwik URL with empty parameters.
             * @param {object} element
             *   A hyperlink element containing an image.
             ...
             */
            var buildContentTrackerUrl = function(url, element, interaction) {
                var src = $('img', element).attr('src');
                var target = $(element).attr('href');
                return url.replace('c_i=', 'c_i=' + encodeURIComponent(interaction))
                    .replace('c_n=', 'c_n=' + encodeURIComponent(src))
                    .replace('c_p=', 'c_p=' + encodeURIComponent(src))
                    .replace('c_t=', 'c_t=' + encodeURIComponent(target))
                    .replace('urlref=', 'urlref=' + encodeURIComponent(document.referrer));
            };

            /**
             * Setup the carousel click callback.
             */
            $('#views_slideshow_cycle_main_front_page_slideshow-panel_pane_1 a', context).once('eff-piwik-content-tracking', function() {
                $.post(buildContentTrackerUrl(settings.effPiwikUrl, this, ''));
                $(this).click(function() {
                    var link = this;
                    $.ajax({
                        url: buildContentTrackerUrl(settings.effPiwikUrl, this, 'click'),
                        // Set a timeout in case Piwik is down.
                        timeout: 500,
                        type: 'POST'
                    }).always(function() {
